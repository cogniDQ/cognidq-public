...
import { restoreDataSource } from '../../services/datasource';
...
export default function RestoreModal({
  ...
  const mutation = useMutation({
    mutationFn: () => restoreDataSource(workspaceId, dataSourceId),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['data-sources', workspaceId] });
      qc.invalidateQueries({ queryKey: ['data-source', workspaceId, dataSourceId] });
      toast.success('Data source restored');
      onSuccess?.();
      onClose();
    },
    onError: () => {
      toast.error('Failed to restore data source');
    },
  });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-gray-800 border border-gray-700 rounded-2xl p-6 w-full max-w-md">
        <h2 className="text-lg font-semibold text-white mb-2">Restore data source?</h2>
        <p className="text-sm text-gray-400 mb-6">
          <strong className="text-white">{dataSourceName}</strong> will be restored and set back
          to active.
        </p>
