/**
 * SetPasswordPage — target of tenant provisioning's activation URL
 * (``{APP_PUBLIC_URL}/auth/set-password?token=...``).
 *
 * The provisioning service emits this URL for newly-created tenant admins
 * whose accounts are in `PENDING` status. The token is a row in
 * ``password_resets``. Posting it to ``POST /auth/password-reset/confirm``
 * sets the password and (via auth_service.reset_password) transitions
 * the account to `ACTIVE` + `email_verified=true`.
 */
...
export default function SetPasswordPage() {
  ...
  useEffect(() => {
    if (!token) setError('Missing activation token in URL.');
  }, [token]);
      ...
      <div className="min-h-screen bg-gradient-to-br from-dark-950 via-dark-900 to-dark-950 flex items-center justify-center px-4">
        <div className="max-w-md w-full glass rounded-lg border border-dark-700 p-8 text-center">
          ...
          <p className="text-gray-400 mb-6">
            Your account is now active. Sign in to access your tenant.
          </p>
          <button
      ...
      <div className="max-w-md w-full space-y-6 relative z-10">
        <div className="text-center">
          ...
          <p className="text-gray-400 text-sm">
            Choose a password to activate your tenant administrator account.
          </p>
        </div>
        ...
        >
          ...
          <p className="text-center text-sm text-gray-500">
            Already activated? <Link to="/auth/login" className="text-primary-400 hover:text-primary-300">Sign in</Link>
          </p>
        </form>
      </div>
    </div>
